...
describe('#813', () => {
  ...
  it('changeData', async () => {
    ...
    const interactions = view.getOptions().interactions;
    ...
    expect(findIndex(interactions, (item) => item.type === 'tooltip')).toBeGreaterThanOrEqual(0);
    // legend active added
    expect(findIndex(interactions, (item) => item.type === 'legend-active')).toBeGreaterThanOrEqual(0);
    // legend filter added
    expect(findIndex(interactions, (item) => item.type === 'legend-filter')).toBeGreaterThanOrEqual(0);
    // duck check
    expect(findIndex(interactions, (item) => item.type === 'duck-action')).toBe(-1);
  });
});
