# G2Plot v3：面向 AI Coding 的可视化组件库
...
v3 重新定义为一个**非 JS 运行时库**的产品：一套**面向 AI Coding 的可视化组件库**，组件由「提示词 + G2 v5 案例代码」组成，供 AI Coding Agent 读取后为开发者生成图表代码。

> 修订记录（2026-09-20）：定位措辞由「图表知识库」修订为「面向 AI Coding 的可视化组件库」——组件库是产品心智，prompt 是组件的实现形态。
