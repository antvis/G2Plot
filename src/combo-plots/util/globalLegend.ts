...
export function getLegendData(viewLayer: ViewLayer){
    const legendItems = [];
    ...
    const marker = geometry.get('shapeFactory').getMarkerStyle(geometry.type,{
...
export function createLegend(items,container,width,canvas){
    const legendCfg = {
        ...
        container:canvas,
        group:container,
        textStyle: {fill:'#000000',fontSize:12},  // 图例项目文本样式
        titleDistance: 10,// 标题和图例项的间距
        autoWrap: true, // 图例项是否自动换行
        itemWidth: 100, // 图例项目宽度
        wordSpacing:3, // 图例项文本同 marker 之间的间距
        itemMarginBottom: 5, // 图例项之间的底部间距
        backgroundPadding: 5,// 背景内边距
