...
export default class GuideLineParser extends LineParser {
    public init(){
        ...
        if(this._getColorMappingField() || this._needParseAttribute('color')) this.parseColor();
    ...
    public parseColor(){
        ...
        if(colorField)  config.fields = colorField;
        if(props.line.style && props.line.style.color){
            config.values = [props.line.style.color];
        }else{
            if(!colorField) colorField = this.config.position.fields;
