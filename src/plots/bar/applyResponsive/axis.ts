...
function responsiveXAxis(plot, props, responsiveTheme) {
  ...
  const region = new BBox(0, 0, width, height);
  const { constraints, rules } = responsiveTheme.axis.y.category.label;
  new Responsive({
    region,
    nodes: shapeNodes,
    constraints,
...
function responsiveYaxis(plot, props, responsiveTheme) {
  ...
  const { constraints, rules } = responsiveTheme.axis.x.linear.label;
  new Responsive({
