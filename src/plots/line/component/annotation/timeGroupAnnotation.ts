...
export default class TimeGroupAnnotation {
  ...
  private _getWeekIntervals() {
    ...
    while (last_sunday < this.max) {
      let _middle: number | string = privious_day + (last_sunday - privious_day) / 2;
      _middle = moment(_middle).format(this.dateFormater);
      texts.push({ value: _middle, text: `第${count}周` });
      intervals.push(last_sunday.format(this.dateFormater));
      privious_day = last_sunday;
      last_sunday = moment(last_sunday).add(7, 'days');
      count++;
    }
    middle = privious_day + (this.max - privious_day) / 2;
  ...
  private _getMonthIntervals() {
    ...
    let middle: moment.Moment | string;

    while (last_month_day < this.max) {
      const _middle: moment.Moment | string = previous_month_day.add(last_month_day.diff(previous_month_day) / 2);
      texts.push({ value: _middle, text: `${last_month + 1}月` });
    ...
    middle = previous_month_day.add(moment(this.max).diff(previous_month_day) / 2);
    middle = moment(middle).format(this.dateFormater);
  ...
  private _getYearIntervals() {
    ...
    while (last_year_day < this.max) {
      const _middle: moment.Moment | string = previous_year_day.add(last_year_day.diff(previous_year_day) / 2);
      texts.push({ value: _middle, text: `${last_year}年` });
    ...
    middle = previous_year_day.add(moment(this.max).diff(previous_year_day) / 2);
    middle = moment(middle).format(this.dateFormater);
