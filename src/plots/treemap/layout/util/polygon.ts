...
function calBox(inputs){
    ...
    const ymin=points[0].y,ymax=points[points.length-1].y;

    if(arguments[1]=="flat") {
        return {top_left:{x:xmin,y:ymin},
...
function getLocalCoords(points,tl/*of bbox*/){
    ...
    for(var i=0; i<points.length; i++){
       var p=points[i];
       var dx= dist2({x:p.x,y:tl.y},tl),
           dy= dist2({x:tl.x,y:p.y},tl);
...
function counter_clockwise(points,bottom_left){
    ...
    ps.sort(function(a,b){
            var dist_a=Math.sqrt(Math.pow((bottom_left.x-a.value.x),2)+Math.pow((bottom_left.y-a.value.y),2)),
                dist_b=Math.sqrt(Math.pow((bottom_left.x-b.value.x),2)+Math.pow((bottom_left.y-b.value.y),2));
            return dist_a-dist_b;
        });
    var startPoint=ps[0].index;
    var pa=points[startPoint].value,
    ...
    var v1=pl.clone().sub(pa),
        v2=pa.clone().sub(pr),
        cross=v1.cross(v2);
    var dir=(cross.z>=0)?1:-1,
        targetPoint=(cross.z>=0)?points.length:0;

    var arr1=points.slice(Math.min(startPoint,targetPoint),Math.max(startPoint,targetPoint)+1);
...
function triangulate_polygon(points){
    ...
    while(ps.length>3 && i<=ps.length-3){
        var tri=new Triangle([ps[i].value,ps[i+1].value,ps[i+2].value]);
